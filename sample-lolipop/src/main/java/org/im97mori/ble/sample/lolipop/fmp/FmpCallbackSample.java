...
import org.im97mori.ble.profile.fmp.central.FindMeProfileCallback;
...
public class FmpCallbackSample extends FindMeProfileMockCallback implements FindMeProfileCallback, BLEServerCallback {

    public static class Builder extends FindMeProfileMockCallback.Builder<FmpCallbackSample> {

        private final SampleCallback mSampleCallback;

        public Builder(@NonNull Context context, SampleCallback sampleCallback) {
            super(context, new IasCallbackSample.Builder(sampleCallback));
            mSampleCallback = sampleCallback;
        }

        @Override
        public FmpCallbackSample build() {
            return new FmpCallbackSample(mContext, mImmediateAlertServiceMockCallbackBuilder.build(), mSampleCallback);
        }
    }
    ...
    public FmpCallbackSample(Context context, SampleCallback sampleCallback) {
        this(context, null, sampleCallback);
    }

    public FmpCallbackSample(Context context, ImmediateAlertServiceMockCallback immediateAlertServiceMockCallback, SampleCallback sampleCallback) {
        super(context, immediateAlertServiceMockCallback);
        mSampleCallback = sampleCallback;
    }
    ...
    public void onScanFinished(@NonNull Set<BluetoothDevice> bluetoothDeviceSet, long timeout, @Nullable Bundle argument) {
        if (!bluetoothDeviceSet.isEmpty() && mSampleCallback instanceof FmpCentralSampleActivity) {
            ((FmpCentralSampleActivity) mSampleCallback).mBluetoothDevice = bluetoothDeviceSet.iterator().next();
        }
