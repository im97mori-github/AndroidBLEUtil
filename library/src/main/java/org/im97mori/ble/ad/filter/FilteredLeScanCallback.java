...
public class FilteredLeScanCallback implements BluetoothAdapter.LeScanCallback {
    ...
    public final void onLeScan(BluetoothDevice device, int rssi, byte[] scanRecord) {
        ...
        if (mAndFilter.isMatched(result)) {
            ...
            BLELogUtils.stackLog("not matched", device, Arrays.toString(scanRecord));
        }
    }
     ...
     * @param result     {@link org.im97mori.ble.ad.AdvertisingDataParser.AdvertisingDataParseResult} instance, created from scanRecord byte array
     */
